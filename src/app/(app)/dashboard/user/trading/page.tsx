...
function TradingUnavailable() {
  return (
    <AppPage maxWidth={760}>
      <SectionCard>
        <EmptyState
          icon={<ShowChartIcon />}
          title="Simulated practice is being prepared"
          description="Practice will use delayed market prices from our own data service. This screen is available for internal development only until that service is live."
        />
      </SectionCard>
    </AppPage>
  );
...
function TradingTerminalDevelopmentPage() {
  ...
  const sidebarContent = (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, overflow: 'auto', p: { xs: 1.5, lg: 0 } }}>
      {/* Order Form */}
      <Paper elevation={0} sx={{ p: 1.5, borderRadius: 2, border: (theme) => `1px solid ${(theme.vars || theme).palette.divider}` }}>
        <OrderForm
    ...
    return (
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, minHeight: 0, overflow: 'hidden' }}>
          ...
          >
            <Box sx={{ p: 1, borderBottom: (theme) => `1px solid ${(theme.vars || theme).palette.divider}` }}>
  ...
  return (
    <Box
      sx={{
        // Fill the AppShell content area (viewport minus the 64px top bar) on desktop.
    ...
    >
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, px: { xs: 1, md: 1.5 }, pb: 1 }}>
        ...
        {initializing ? (
          ...
          <Paper sx={{ p: 3, borderRadius: 2, border: (theme) => `1px solid ${(theme.vars || theme).palette.divider}` }}>
